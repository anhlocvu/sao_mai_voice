...
7. **Sửa lỗi đọc thiếu câu (mất chữ "space" và tên ứng dụng khi chuyển tiêu điểm)**:
    - Khai báo `supportedNotifications = {synthDoneSpeaking}` trong driver và bổ sung import chúng từ `synthDriverHandler` ở đầu file để tránh NameError. Đồng thời import `IndexCommand` từ `speech.commands` để khắc phục lỗi `AttributeError` khi so khớp loại phần tử trong `speechSequence`.
   - Phát hiện engine Sao Mai VNVoice không hỗ trợ thẻ tự đóng `<bookmark mark="..."/>` của SAPI5, dẫn đến việc bị câm lặng/im hoàn toàn khi gặp index command. Đã loại bỏ việc dịch bookmark trong `speechSequence` và tập trung vào đồng bộ stream qua sự kiện `EndStream`.
   - Driver 64-bit sử dụng `queueHandler.queueFunction` để đẩy các thông báo `synthDoneSpeaking.notify()` về luồng chính của NVDA khi nhận sự kiện `"end"` từ bridge. Điều này giúp NVDA core đồng bộ hóa hoàn hảo với tiến độ đọc của engine, giải quyết triệt để lỗi bỏ sót/nuốt chữ.
   - Để tránh xung đột khi NVDA gửi nhiều câu đọc nối tiếp nhau rất nhanh (như container và item, hoặc từ và khoảng trắng), chúng ta đã loại bỏ cờ `SPF_PURGEBEFORESPEAK` (giá trị `2`) khỏi lệnh `speak` trong `bridge.py` và chỉ giữ lại cờ `1 | 8` (bất đồng bộ và XML). Điều này cho phép SAPI5 tự động xếp hàng (speech queue) các câu đọc một cách tuần tự, giúp các câu phát ra liền mạch mà không bị câu sau đè/hủy câu trước.
   - Khi nhận lệnh đổi cấu hình mà không có văn bản đọc (`text` trống), bridge sẽ không gọi SAPI5 `Speak` nữa mà chỉ cập nhật trực tiếp các thuộc tính COM (`Voice`, `Rate`, `Volume`), giúp thay đổi có hiệu lực ngay lập tức mà không làm ngắt quãng câu đang đọc dở.
   - Khi nhận lệnh `cancel`, driver luôn chủ động đặt `self._is_speaking = False` và gọi `synthDoneSpeaking.notify(self)` ngay trên driver để giải phóng NVDA core lập tức. Đồng thời bridge cũng luôn gọi `Speak("", 2)` để dọn sạch hàng đợi nói của SAPI5.
   - Khắc phục race condition của sự kiện `EndStream` cũ: Trong bridge, khi nhận lệnh `cancel`, ta đặt ngay `self.is_speaking = False` để vô hiệu hóa sự kiện `EndStream` cũ. Trong `EndStream` của sink, ta chỉ gửi `"end"` về driver nếu `self.is_speaking` là `True`. Đồng thời giữ lại lệnh `cancel` khi gộp lệnh có cancel (`has_cancel = True`) để engine dọn dẹp sạch sẽ trước khi đọc câu mới.
8. **Tối ưu hóa độ nhạy phản hồi (loại bỏ trễ vài ms và sửa deadlock COM STA)**:
   - Sửa lỗi deadlock COM STA trong `bridge.py`: thay đổi vòng lặp chính của `bridge.py` bằng cách loại bỏ `self.cmd_queue.get(timeout=0.001)` (gây treo luồng giao tiếp COM STA Windows) và thay bằng cuộc gọi không block `self.cmd_queue.get_nowait()`.
   - Sử dụng `comtypes.client.PumpEvents(0.002)` (chờ 2ms và xử lý thông điệp Windows) làm điểm dừng nhàn rỗi duy nhất cho vòng lặp chính. Cách này giữ CPU sử dụng ở mức 0% và đảm bảo kênh COM STA luôn thông suốt, không bao giờ bị nghẽn.
   - Bổ sung cơ chế **Gộp lệnh trong hàng đợi (Command Coalescing)**: Hỗ trợ gộp dồn toa các cài đặt (voice, rate, volume) một cách thông minh. Thuật toán mới được tinh chỉnh để giữ lại toàn bộ các câu `speak` nối tiếp nhau (không có lệnh `cancel` xen giữa) nhằm gửi cho SAPI5 xếp hàng đọc tuần tự, và chỉ thực hiện xóa các câu tích lũy trước đó khi nhận được lệnh `cancel`. Giúp engine hoạt động mượt mà, nhạy bén và giải quyết triệt để lỗi mất câu nối tiếp khi di chuyển ra ngoài desktop hoặc gõ chữ.

9. **Sửa lỗi nuốt câu sau khi chuyển tiêu điểm nhanh (Desktop list/Item)**:
    - Phát hiện Race Condition xảy ra khi NVDA gửi hai câu nói rất nhanh (ví dụ `"desktop list"` và sau đó là `"This PC 25 of 92"`). Do độ trễ IPC, khi câu đầu kết thúc, bridge gửi `"end"` lên và driver báo `synthDoneSpeaking`. Ngay lập tức NVDA gửi lệnh `cancel()` xuống để dọn dẹp trạng thái cũ, khiến SAPI5 ở tiến trình con 32-bit hủy luôn câu thứ hai đang nằm trong hàng đợi hoặc vừa mới phát.
    - Đã sửa bằng cách triển khai cơ chế **Theo dõi Stream đang hoạt động (`active_streams`)** và kết hợp kiểm tra trạng thái **`RunningState`** thực tế của SpVoice trong `bridge.py`.
    - Khắc phục race condition của các stream cũ bị hủy: Khi một stream bị hủy (ví dụ do lệnh `cancel` của NVDA), SAPI5 vẫn có thể kích hoạt sự kiện `EndStream` cho stream đó. Chúng ta đã tinh chỉnh hàm `EndStream` để **bỏ qua ngay lập tức** nếu `StreamNumber` của sự kiện không nằm trong `active_streams` hiện tại. Điều này ngăn chặn việc sự kiện của stream cũ vô tình xóa dấu vết của stream mới đang chuẩn bị phát và báo `"end"` sai thời điểm về driver.
    - Bridge hiện tại chỉ gửi sự kiện `"end"` về driver 64-bit khi danh sách `active_streams` đã trống hoàn toàn và thuộc tính `self.voice.Status.RunningState` báo hiệu SpVoice đã nhàn rỗi (giá trị `1`). Điều này ngăn chặn việc driver báo `synthDoneSpeaking` quá sớm, giúp bảo vệ các câu nói tiếp theo không bị NVDA gửi lệnh `cancel()` hủy bỏ giữa chừng.
